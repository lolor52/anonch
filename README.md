...
- вход через modal-окно `VK ID` и `Yandex ID` без внешних JS SDK
...
Сейчас основной сценарий входа открывается в modal-окне поверх текущей страницы. Маршрут `/auth/` оставлен только как служебный launcher для перенаправления в этот modal.

Текущий конфиг переведён в `real`. Если обязательные поля оставить пустыми, интерфейс не падает: modal показывает понятный статус конфигурации, а при попытке входа пользователь получает явное сообщение об ошибке.
...
По умолчанию `VK ID` и `Yandex ID` работают в `real` режиме. Для запуска реального входа нужно:

1. заполнить `appId` для `VK ID` и `clientId` для `Yandex ID` в [src/config/auth-providers.js](src/config/auth-providers.js)
2. зарегистрировать корректные `redirect URI` у обоих провайдеров
3. запустить `python3 -m http.server 4173 --bind 127.0.0.1`
4. открыть сайт и нажать `Вход`
