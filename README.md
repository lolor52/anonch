...
- вход через `VK ID` и `Yandex ID` с `mock` и `real` режимами без внешних JS SDK
...
- `real` — браузерная интеграция с заполненными ключами и redirect URI
...
Локальный вход по логину в проекте не используется.

Важно:
- для `VK ID` в проекте реализован актуальный web flow без SDK: `code + PKCE`, затем обмен кода на токен и чтение профиля через `VK ID API`
- для `Yandex ID` в проекте оставлен frontend-only flow с минимальными правами `login:info login:avatar`; токен используется только на время чтения профиля и не сохраняется
- сам Яндекс не рекомендует хранить токен в браузере, поэтому `Yandex ID real` подходит только как ограниченный frontend-only сценарий
...
src/features/auth/    провайдеры входа, guards, store
...
- внешний OAuth-токен не сохраняется в `localStorage`: после чтения профиля в браузере остаётся только локальная сессия приложения
- без backend возможен только frontend-only сценарий: `mock` режим или ограниченный клиентский OAuth flow, если провайдер его допускает
- для полноценной production-авторизации и безопасной работы с пользовательскими данными нужен backend
